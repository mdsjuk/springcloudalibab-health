package com.uber.engsec.dp.examples
...
import com.uber.engsec.dp.schema.{Redisutils, Schema}
...
/** A simple example demonstrating query rewriting for differential privacy.
  */
object QueryRewritingExample  {
  // Use the table schemas and metadata defined by the test classes
//  System.setProperty("schema.config.path", "schema.yaml")
//  val database = Schema.getDatabase("test")
  val database=Redisutils.get_Database("test")
  ...
  def elasticSensitivityExample():String = {
   ...
   println("***重写sql" + rewrittenQuery.toSql() + "***")
    "***重写sql" + rewrittenQuery.toSql() + "***"
  }
